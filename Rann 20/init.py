...
from data import eventdetailsEn
	...
	# try:
		...
		if session["logged_in"]:
			...
			if path[0]=="home":
				...
				return render_template("main.html",userdata=client_data,event_list=event_list,logged_in=True)
			#payment changes
				...
				try:
					if path[1]=="Pool":
						...
						return redirect("https://www.kiet.edu/erp-apis/index.php/payment/do_transaction?APP_KEY=RANN_20_QBZJA&CUST_ID="+str(pid)+str(team.team_id)+"&TXN_AMOUNT="+str(team.amt_paid)+"&CALLBACK_URL=http://rann.kiet.edu/verify-payment")
					elif path[1]=="Cricket":
						pid=789588
...
def EventsList(eventname):
	...
	#new entry for 2020
		...
		try:
			if session["logged_in"]:
				client_data=Competitors.query.filter_by(email=session["username"]).first()
				if Yoga.query.filter_by(captain_id=client_data.c_id).count()>0:
					eventdata=Events.query.filter_by(name='Yoga').first()
					participated_in=Yoga.query.filter_by(captain_id=client_data.c_id).all()
					return render_template("events.html",event=eventdata,fname="images/yoga.jpg",redir=redir,participated_in=participated_in)
				else:
					eventdata=Events.query.filter_by(name='Yoga').first()
					return render_template("events.html",event=eventdata,fname="images/fifa.jpg",redir=redir,participated_in=[])
			else:
		...
		try:
			if session["logged_in"]:
				client_data=Competitors.query.filter_by(email=session["username"]).first()
				if Karate.query.filter_by(captain_id=client_data.c_id).count()>0:
					eventdata=Events.query.filter_by(name='Karate').first()
					participated_in=Karate.query.filter_by(captain_id=client_data.c_id).all()
					return render_template("events.html",event=eventdata,fname="images/fifa.jpg",redir=redir,participated_in=participated_in)
				else:
					eventdata=Events.query.filter_by(name='Karate').first()
					return render_template("events.html",event=eventdata,fname="images/fifa.jpg",redir=redir,participated_in=[])
		...
		try:
			if session["logged_in"]:
				client_data=Competitors.query.filter_by(email=session["username"]).first()
				if Athletics.query.filter_by(captain_id=client_data.c_id).count()>0:
					eventdata=Events.query.filter_by(name='Athletics').first()
					participated_in=Athletics.query.filter_by(captain_id=client_data.c_id).all()
					return render_template("events.html",event=eventdata,fname="images/fifa.jpg",redir=redir,participated_in=participated_in)
				else:
					eventdata=Events.query.filter_by(name='Athletics').first()
					return render_template("events.html",event=eventdata,fname="images/fifa.jpg",redir=redir,participated_in=[])
